...
def validate_deeplab_model_tensorrt(engine, image_files, class_names, do_crf, label_file, loop_count, output_path):
    # get I/O tensor info
...
def validate_deeplab_model_onnx(model, image_files, class_names, do_crf, label_file, loop_count, output_path):
    input_tensors = []
...
def main():
    parser = argparse.ArgumentParser(description='validate Deeplab GPU model (onnx/engine) with image')
    ...
    if args.model_path.endswith('.engine'):
        # support TensorRT engine model
        validate_deeplab_model_tensorrt(model, image_files, class_names, args.do_crf, args.label_file, args.loop_count, args.output_path)

    elif args.model_path.endswith('.onnx'):
        # support of ONNX model
        validate_deeplab_model_onnx(model, image_files, class_names, args.do_crf, args.label_file, args.loop_count, args.output_path)
